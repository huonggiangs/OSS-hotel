...
async function main() {
  ...
  async function upsertPropertyUser(email: string, fullName: string, role: string) {
    await client.query(
      `INSERT INTO property_users (id, property_id, tenant_id, email, password_hash, full_name, role)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       ON CONFLICT (email) DO NOTHING`,
      [randomUUID(), PROPERTY_ID, TENANT_ID, email, passwordHash, fullName, role]
    );
  }
  await upsertPropertyUser("owner@anio-riverside.local", "Nguyễn Văn Chủ", "OWNER");
  await upsertPropertyUser("manager@anio-riverside.local", "Lê Thảo", "MANAGER");
  await upsertPropertyUser("reception@anio-riverside.local", "Trần Thị Mai", "RECEPTIONIST");
  await upsertPropertyUser("housekeeping@anio-riverside.local", "Nguyễn Văn Bình", "HOUSEKEEPING");
  ...
  console.log("Seed hoàn tất. Tài khoản demo cấp cơ sở (mật khẩu chung: ChangeMe123!):");
  console.log("  - owner@anio-riverside.local (OWNER)");
  console.log("  - manager@anio-riverside.local (MANAGER)");
  console.log("  - reception@anio-riverside.local (RECEPTIONIST)");
  console.log("  - housekeeping@anio-riverside.local (HOUSEKEEPING)");
