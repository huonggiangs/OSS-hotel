...
export function HousekeepingSentModal({ room, onClose, onChanged }: { room: RoomCard; onClose: () => void; onChanged: () => void }) {
  ...
  async function markClean() {
    ...
    setError(null);
    try {
      await api.patch(`/api/v1/rooms/${room.id}`, { status: "VACANT", powerOn: false });
      onChanged();
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-[rgba(23,26,31,.45)]">
      <div className="w-[380px] rounded-[14px] bg-white p-6 text-center">
        ...
        <p className="m-0 mb-[18px] text-[13px] text-pms-muted">Xác nhận để chuyển phòng sang “Trống, sạch”. Tổng quan sẽ nhận trạng thái mới từ cùng dữ liệu này.</p>
