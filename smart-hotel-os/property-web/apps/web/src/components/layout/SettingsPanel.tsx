  ...
  return (
    <div className="flex max-h-full w-[264px] flex-shrink-0 flex-col overflow-y-auto border-r border-pms-border bg-white p-3">
      <div className="flex items-center justify-between px-2 pb-4 pt-1">
        <span className="text-[16px] font-bold">{t("Cài đặt")}</span>
        <div
          onClick={onClose}
          title="Ẩn menu"
      ...
      {settingsTree.map((grp) => (
        <div key={grp.title}>
          <div className="px-2 pb-1.5 pt-3 text-[11px] font-bold uppercase tracking-wide text-pms-muted-2">{t(grp.title)}</div>
          {grp.items.map((it) => {
