  ...
  return (
    <div className="rounded-xl bg-white px-5 pb-5 pt-4 shadow-card">
      ...
      <div className="mb-4 rounded-xl bg-pms-bg px-5 py-4">
        ...
        <div className="mb-[18px] grid grid-cols-4 gap-3">{cards.map((card) => <div key={card.label} className="rounded-[10px] bg-white p-3"><span className="text-[11px] text-pms-muted">{card.label}</span><b className="mt-1.5 block text-[18px]" style={{ color: card.color }}>{card.value}</b></div>)}</div>
