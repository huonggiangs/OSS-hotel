...
export default function EmailPage() {
  ...
  return <div><div className="min-w-0 rounded-xl bg-white p-4 shadow-card sm:p-6">
    <div className="mb-6 flex flex-wrap gap-x-7 gap-y-1 border-b border-pms-border text-[14px]">{([ ["settings", "Cài đặt email"], ["content", "Nội dung email"] ] as const).map(([key, label]) => <button key={key} type="button" className="cursor-pointer border-0 bg-transparent pb-3 font-semibold" style={{ color: tab === key ? "#284AB1" : "#777E90", borderBottom: `2px solid ${tab === key ? "#284AB1" : "transparent"}` }} onClick={() => setTab(key)}>{label}</button>)}</div>
